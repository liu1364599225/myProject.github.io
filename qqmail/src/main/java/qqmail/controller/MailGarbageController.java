...
public class MailGarbageController {
	@Autowired
	private IGarbageService service; 
	
	
	
	@RequestMapping("/readAll_Garbage.do")
	public ModelAndView readAll_Garbage(int pageNum,HttpSession session){
		ModelAndView mv = new ModelAndView();
		//-------------------跳转页面----------------------------------
		User u = (User) session.getAttribute("user");
		int user = u.getUser_id();
		List<MailPro> list8 = service.readAll_Garbage_Count("true",user);//根据状态lnbox为true，send_mail_id查询
		...
		if(pageNum < pageCount){
			...
			list = service.readAll_Garbage("true",user,start,start+pageSize);//根据状态lnbox为true，send_mail_id查询
		}else{
		...
		List<MailPro> list5 =  service.readAll_GarbagePro("true", user);
		mv.addObject("noread_list_size", list5.size());
		mv.setViewName("/WEB-INF/jsp/garbage.jsp");
		return mv;
	}
	
	
	//-------------------跳转二级页面-------------------------------
	@RequestMapping("/second3.do")
	@ResponseBody
	public ModelAndView second3(int time1,int sendId){
		ModelAndView mv = new ModelAndView();
		//List<MailPro> list8 = service.readAll_Inbox_Count("true",1115241874);//查询到所以的数据
		MailPro mailPro = service.readAll_Garbage_ById("true",sendId, time1);//查看对应邮件
	...
	//--------------------------举报------------------------------------------
	@RequestMapping("/report3.do") //加入垃圾箱
	@ResponseBody
	public Object report3(int time,int id,HttpSession session){
		Status status = service.readBySendId(id,time);
		service.updateReport("true", status.getMail_status_id());//加入垃圾箱
		DataDis di = new DataDis();
		User u = (User) session.getAttribute("user");
		int user = u.getUser_id();
		List<MailPro> list1 = service.readAll_Garbage("true",user,0,3);//根据状态lnbox为true，send_mail_id查询
		List list = di.getNum(list1);
		List<MailPro> list5 =  service.readAll_GarbagePro("true", user);
		List<MailPro> list6 = service.readAll_Garbage("true",user,0,3);
		list.add(list6.size());
		return list;
	}
	
	@RequestMapping("/delete4.do") //删除邮件状态
	@ResponseBody
	public Object delete4(int time,int id,HttpSession session){
		Status status = service.readBySendId(id,time);
	...
	@RequestMapping("/deleteTrue3.do")
	@ResponseBody
	public Object deleteTrue3(int time,int id,HttpSession session){ //彻底删除文件
		Status status = service.readBySendId(id,time);
	...
	@RequestMapping("/allReadFun3.do")
	@ResponseBody
	public Object allReadFun3(int time,int id,HttpSession session){ //更新为已读状态
		Status status = service.readBySendId(id,time);
	...
	//-------------------------更新为未读状态--------------------------------------------------
	@RequestMapping("/noReadFun3.do")
	@ResponseBody
	public Object noReadFun3(int time,int id,HttpSession session){ //更新为已读状态
		Status status = service.readBySendId(id,time);
	...
	@RequestMapping("/asend3.do")
	@ResponseBody
	public Object asend3(int time,int id,HttpSession session){ //更新为已读状态
		Status status = service.readBySendId(id,time);
	...
	//-----------------------更新为收件箱---------------------------------------
		@RequestMapping("/inbox3.do")
		@ResponseBody
		public Object inbox3(int time,int id,HttpSession session){ //更新为已读状态
			Status status = service.readBySendId(id,time);
