...
public class MailInboxController {
	...
	@RequestMapping("/readAll_Inbox.do")
	public ModelAndView readAll_Inbox(int pageNum,HttpSession session){
		ModelAndView mv = new ModelAndView();
		//-------------------跳转页面----------------------------------
		User u = (User) session.getAttribute("user");
		int user = u.getUser_id();
		List<MailPro> list8 = service.readAll_Inbox_Count("true",user);//根据状态lnbox为true，send_mail_id查询
		...
		if(pageNum < pageCount){
			...
			list = service.readAll_Inbox("true",user,start,start+pageSize);//根据状态lnbox为true，send_mail_id查询
		}else{
		...
		List<MailPro> list5 =  service.readAll_InboxPro("true", user);
		mv.addObject("noread_list_size", list5.size());
		mv.setViewName("/WEB-INF/jsp/lnbox.jsp");
		
		return mv;
	}
	
	@RequestMapping("/mailColor.do")
	@ResponseBody
	public Object mailColor(String name,int id,int time){//ajax拿到邮件图标的颜色rbg、发件人id（邮箱号）、发件时间
		if(name.equals("rgb(189, 183, 107)")){
	...
	@RequestMapping("/starColor.do")
	@ResponseBody
	public Object starColor(String color,int id,int time){//ajax拿到邮件图标的颜色rbg、发件人id（邮箱号）、发件时间
		
		if(color.equals("rgb(250, 208, 77)")){ //更新为星标标记状态
			Status status = service.readBySendId(id,time);
			service.updateStar("true",status.getMail_status_id());
		}else{ 			//更新为 不是星标状态
			Status status = service.readBySendId(id,time);
			service.updateStar("false",status.getMail_status_id());
		}
		return color;
	}
	//------------------------转发---------------------------------------------
	@RequestMapping("/transpond.do") //加入垃圾箱
	@ResponseBody
	public Object transpond(String time,int id,HttpSession session){
		return "time"+time+"send_id"+id;
	}
	//-------------------跳转二级页面-------------------------------
	@RequestMapping("/second.do")
	@ResponseBody
	public ModelAndView second(int time1,int sendId){
		ModelAndView mv = new ModelAndView();
		//List<MailPro> list8 = service.readAll_Inbox_Count("true",1115241874);//查询到所以的数据
		MailPro mailPro = service.readAll_Inbox_ById("true",sendId, time1);//查看对应邮件
		Date data = mailPro.getSend_time();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		String dateStr = sdf.format(data);
		mailPro.setDate(dateStr);
		mv.addObject("mailPro", mailPro);
		System.out.println(mailPro);
		mv.addObject("url_back", "/readAll_Inbox.do");
	...
	//--------------------------举报------------------------------------------
	@RequestMapping("/report.do") //加入垃圾箱
	@ResponseBody
	public Object report(int time,int id,HttpSession session){
		Status status = service.readBySendId(id,time);
		...
		User u = (User) session.getAttribute("user");
		int user = u.getUser_id();
		List<MailPro> list1 = service.readAll_Inbox("true",user,0,3);//根据状态lnbox为true，send_mail_id查询
		List list = di.getNum(list1);
		List<MailPro> list5 =  service.readAll_InboxPro("true", 1115241874);
		List<MailPro> list6 = service.readAll_Inbox("true",1115241874,0,3);
		list.add(list6.size());
		return list;
	}
	
	@RequestMapping("/delete1.do") //删除邮件状态
	@ResponseBody
	public Object delete1(int time,int id,HttpSession session){
		Status status = service.readBySendId(id,time);
		...
		List list = di.getNum(list1);
		List<MailPro> list5 =  service.readAll_InboxPro("true", user);
		List<MailPro> list6 = service.readAll_Inbox("true",user,0,3);
		list.add(list6.size());
		return list;
	}
	
	@RequestMapping("/deleteTrue.do")
	@ResponseBody
	public Object deleteTrue(int time,int id,HttpSession session){ //彻底删除文件
		Status status = service.readBySendId(id,time);
	...
	@RequestMapping("/allReadFun.do")
	@ResponseBody
	public Object allReadFun(int time,int id,HttpSession session){ //更新为已读状态
		Status status = service.readBySendId(id,time);
		service.update("true","false",status.getMail_status_id());
		User u = (User) session.getAttribute("user");
		int user = u.getUser_id();
	...
	//-------------------------更新为未读状态--------------------------------------------------
	@RequestMapping("/noReadFun.do")
	@ResponseBody
	public Object noReadFun(int time,int id,HttpSession session){ //更新为已读状态
		Status status = service.readBySendId(id,time);
	...
	@RequestMapping("/asend.do")
	@ResponseBody
	public Object asend(int time,int id,HttpSession session){ //更新为已读状态
		Status status = service.readBySendId(id,time);
