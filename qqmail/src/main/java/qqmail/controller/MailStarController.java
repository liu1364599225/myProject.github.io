...
import qqmail.service.impl.IStarServiceImpl;
import qqmail.util.DataDis;
import qqmail.util.DateUtil;

@Controller
public class MailStarController {
@Autowired
private IStarServiceImpl service;




@RequestMapping("/readAll_Star.do")
public ModelAndView readAll_Star(int pageNum,HttpSession session){
	ModelAndView mv = new ModelAndView();
	//-------------------跳转页面----------------------------------
	User u = (User) session.getAttribute("user");
	int user = u.getUser_id();
	List<MailPro> list8 = service.readAll_Star_Count("true",user);//根据状态lnbox为true，send_mail_id查询
	...
	if(pageNum < pageCount){
		...
		list = service.readAll_Star("true",user,start,start+pageSize);//根据状态lnbox为true，send_mail_id查询
	}else{
	...
	List<MailPro> list5 =  service.readAll_StarPro("true", user);
	mv.addObject("noread_list_size", list5.size());
	System.out.println(list+"状态");
	mv.setViewName("/WEB-INF/jsp/star.jsp");
	
	return mv;
}

//-------------------跳转二级页面-------------------------------
@RequestMapping("/second1.do")
@ResponseBody
public ModelAndView second1(int time1,int sendId){
	ModelAndView mv = new ModelAndView();
	System.out.println(time1+"&"+sendId);
	//List<MailPro> list8 = service.readAll_Inbox_Count("true",1115241874);//查询到所以的数据
	MailPro mailPro = service.readAll_Star_ById("true",sendId, time1);//查看对应邮件
...
//--------------------------举报------------------------------------------
@RequestMapping("/report1.do") //加入垃圾箱
@ResponseBody
public Object report1(int time,int id,HttpSession session){
	Status status = service.readBySendId(id,time);
	service.updateReport("true", status.getMail_status_id());//加入垃圾箱
	DataDis di = new DataDis();
	User u = (User) session.getAttribute("user");
	int user = u.getUser_id();
	List<MailPro> list1 = service.readAll_Star("true",user,0,3);//根据状态lnbox为true，send_mail_id查询
	List list = di.getNum(list1);
	List<MailPro> list5 =  service.readAll_StarPro("true", user);
	List<MailPro> list6 = service.readAll_Star("true",user,0,3);
	list.add(list6.size());
	return list;
}

@RequestMapping("/delete2.do") //删除邮件状态
@ResponseBody
public Object delete2(int time,int id,HttpSession session){
	Status status = service.readBySendId(id,time);
...
@RequestMapping("/deleteTrue1.do")
@ResponseBody
public Object deleteTrue1(int time,int id,HttpSession session){ //彻底删除文件
	Status status = service.readBySendId(id,time);
...
@RequestMapping("/allReadFun1.do")
@ResponseBody
public Object allReadFun1(int time,int id,HttpSession session){ //更新为已读状态
	Status status = service.readBySendId(id,time);
...
//-------------------------更新为未读状态--------------------------------------------------
@RequestMapping("/noReadFun1.do")
@ResponseBody
public Object noReadFun1(int time,int id,HttpSession session){ //更新为已读状态
	Status status = service.readBySendId(id,time);
...
@RequestMapping("/asend1.do")
@ResponseBody
public Object asend1(int time,int id,HttpSession session){ //更新为已读状态
	Status status = service.readBySendId(id,time);
...
//-----------------------更新为收件箱---------------------------------------
	@RequestMapping("/inbox1.do")
	@ResponseBody
	public Object inbox1(int time,int id,HttpSession session){ //更新为已读状态
		Status status = service.readBySendId(id,time);
