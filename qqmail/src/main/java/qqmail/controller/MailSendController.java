...
public class MailSendController {
	...
	@RequestMapping("/readAll_Send.do")
	public ModelAndView readAll_Send(int pageNum,HttpSession session){
		ModelAndView mv = new ModelAndView();
		//-------------------跳转页面----------------------------------
		User u = (User) session.getAttribute("user");
		int user = u.getUser_id();
		List<MailPro> list8 = service.readAll_Send_Count("true",user);//根据状态lnbox为true，send_mail_id查询
		...
		if(pageNum < pageCount){
			...
			list = service.readAll_Send("true",user,start,start+pageSize);//根据状态lnbox为true，send_mail_id查询
		}else{
		...
		List<MailPro> list5 =  service.readAll_SendPro("true", user);
		mv.addObject("noread_list_size", list5.size());
		mv.setViewName("/WEB-INF/jsp/send.jsp");
		return mv;
	}
	
	
	//-------------------跳转二级页面-------------------------------
	@RequestMapping("/second5.do")
	@ResponseBody
	public ModelAndView second5(int time1,int sendId){
		ModelAndView mv = new ModelAndView();
		//List<MailPro> list8 = service.readAll_Inbox_Count("true",1115241874);//查询到所以的数据
		MailPro mailPro = service.readAll_Send_ById("true",sendId, time1);//查看对应邮件
	...
	//--------------------------举报------------------------------------------
	@RequestMapping("/report5.do") //加入垃圾箱
	@ResponseBody
	public Object report5(int time,int id,HttpSession session){
		Status status = service.readBySendId(id,time);
		service.updateReport("true", status.getMail_status_id());//加入垃圾箱
		DataDis di = new DataDis();
		User u = (User) session.getAttribute("user");
		int user = u.getUser_id();
		List<MailPro> list1 = service.readAll_Send("true",user,0,3);//根据状态lnbox为true，send_mail_id查询
		List list = di.getNum(list1);
		List<MailPro> list5 =  service.readAll_SendPro("true", user);
		List<MailPro> list6 = service.readAll_Send("true",user,0,3);
		list.add(list6.size());
		return list;
	}
	
	@RequestMapping("/delete6.do") //删除邮件状态
	@ResponseBody
	public Object delete6(int time,int id,HttpSession session){
		Status status = service.readBySendId(id,time);
	...
	@RequestMapping("/deleteTrue5.do")
	@ResponseBody
	public Object deleteTrue5(int time,int id,HttpSession session){ //彻底删除文件
		Status status = service.readBySendId(id,time);
	...
	@RequestMapping("/allReadFun5.do")
	@ResponseBody
	public Object allReadFun5(int time,int id,HttpSession session){ //更新为已读状态
		Status status = service.readBySendId(id,time);
	...
	//-------------------------更新为未读状态--------------------------------------------------
	@RequestMapping("/noReadFun5.do")
	@ResponseBody
	public Object noReadFun5(int time,int id,HttpSession session){ //更新为已读状态
		Status status = service.readBySendId(id,time);
	...
	//-----------------------更新为收件箱---------------------------------------
	@RequestMapping("/inbox5.do")
	@ResponseBody
	public Object inbox5(int time,int id,HttpSession session){ //更新为已读状态
		Status status = service.readBySendId(id,time);
	...
	@RequestMapping("/asend5.do")
	@ResponseBody
	public Object asend5(int time,int id,HttpSession session){ //更新为已读状态
		Status status = service.readBySendId(id,time);
