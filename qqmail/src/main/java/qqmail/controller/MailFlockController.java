...
@Controller
public class MailFlockController {
	@Autowired
	private IFlockService service; 
	
	
	
	@RequestMapping("/readAll_Flock.do")
	public ModelAndView readAll_Inbox(int pageNum,HttpSession session){
		ModelAndView mv = new ModelAndView();
		//-------------------跳转页面----------------------------------
		User u = (User) session.getAttribute("user");
		int user = u.getUser_id();
		List<MailPro> list8 = service.readAll_Flock_Count("true",user);//根据状态lnbox为true，send_mail_id查询
		...
		if(pageNum < pageCount){
			...
			list = service.readAll_Flock("true",user,start,start+pageSize);//根据状态lnbox为true，send_mail_id查询
		}else{
		...
		List<MailPro> list5 =  service.readAll_FlockPro("true", user);
		mv.addObject("noread_list_size", list5.size());
		System.out.println(list+"状态");
		mv.setViewName("/WEB-INF/jsp/flock.jsp");
		
		return mv;
	}
	
	@RequestMapping("/mailColor7.do")
	@ResponseBody
	public Object mailColor7(String name,int id,int time){//ajax拿到邮件图标的颜色rbg、发件人id（邮箱号）、发件时间
	...
	//-------------------跳转二级页面-------------------------------
	@RequestMapping("/second7.do")
	@ResponseBody
	public ModelAndView second7(int time1,int sendId){
		ModelAndView mv = new ModelAndView();
		System.out.println(time1+"&"+sendId);
		//List<MailPro> list8 = service.readAll_Inbox_Count("true",1115241874);//查询到所以的数据
		MailPro mailPro = service.readAll_Flock_ById("true",sendId, time1);//查看对应邮件
	...
	//--------------------------举报------------------------------------------
	@RequestMapping("/report7.do") //加入垃圾箱
	@ResponseBody
	public Object report7(int time,int id,HttpSession session){
		Status status = service.readBySendId(id,time);
		service.updateReport("true", status.getMail_status_id());//加入垃圾箱
		DataDis di = new DataDis();
		User u = (User) session.getAttribute("user");
		int user = u.getUser_id();
		List<MailPro> list1 = service.readAll_Flock("true",user,0,3);//根据状态lnbox为true，send_mail_id查询
		List list = di.getNum(list1);
		List<MailPro> list5 =  service.readAll_FlockPro("true", user);
		List<MailPro> list6 = service.readAll_Flock("true",user,0,3);
		list.add(list6.size());
		return list;
	}
	
	@RequestMapping("/delete8.do") //删除邮件状态
	@ResponseBody
	public Object delete8(int time,int id,HttpSession session){
		Status status = service.readBySendId(id,time);
	...
	@RequestMapping("/deleteTrue7.do")
	@ResponseBody
	public Object deleteTrue7(int time,int id,HttpSession session){ //彻底删除文件
		Status status = service.readBySendId(id,time);
	...
	@RequestMapping("/allReadFun7.do")
	@ResponseBody
	public Object allReadFun7(int time,int id,HttpSession session){ //更新为已读状态
		Status status = service.readBySendId(id,time);
		service.update("true","false",status.getMail_status_id());
		User u = (User) session.getAttribute("user");
		int user = u.getUser_id();		List<MailPro> list5 =  service.readAll_FlockPro("true", user);
		List<MailPro> list6 = service.readAll_Flock("true",user,0,3);
		List list = new ArrayList();
		list.add(list5.size());
		list.add(list6.size());
	...
	//-------------------------更新为未读状态--------------------------------------------------
	@RequestMapping("/noReadFun7.do")
	@ResponseBody
	public Object noReadFun7(int time,int id,HttpSession session){ //更新为已读状态
		Status status = service.readBySendId(id,time);
	...
	@RequestMapping("/asend7.do")
	@ResponseBody
	public Object asend7(int time,int id,HttpSession session){ //更新为已读状态
		Status status = service.readBySendId(id,time);
	...
	//-----------------------更新为收件箱---------------------------------------
		@RequestMapping("/inbox7.do")
		@ResponseBody
		public Object inbox7(int time,int id,HttpSession session){ //更新为已读状态
			Status status = service.readBySendId(id,time);
