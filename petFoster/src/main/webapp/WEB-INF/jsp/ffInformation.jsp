...
<body>
	<div id="top" class="container">
		...
		<div id="top-navigation">
			<div id="top-navigation-name">账户名:${fosterFamily.getfAccountname() }</div>
			<a class="btn btn-default btn-lg" href="${pageContext.servletContext.contextPath }/psAndffOrderPage.pet?oFosterplaceid=${fosterFamily.getfId() }&oFosterstyle=${oFosterstyle}"> 我的订单 </a>
			<a class="btn btn-danger btn-lg" href="${pageContext.servletContext.contextPath }/deleteffAccount.pet?fId=${fosterFamily.getfId() }"> 注销此账号 </a>
	...
	<div class="container">
		<div class="col-md-6 col-md-offset-3" id="center">
			<div class="container-fluid information-title">寄养家庭信息</div>
			<div class="row">
				<div class="col-md-10 col-md-offset-1" style="margin-top: 20px;">
					<div class="information-row">
						<span class="information-row-title">账户名：</span> <span>${fosterFamily.getfAccountname() }</span>
					</div>
					<div class="information-row">
						<span class="information-row-title">收养人姓名：</span> <span>${fosterFamily.getfAdoptername() }</span>
					</div>
					<div class="information-row">
						<span class="information-row-title">收养人性别：</span> <span>${fosterFamily.getfAdoptersex() }</span>
					</div>
					<div class="information-row">
						<span class="information-row-title">寄养经历：</span> <span>${fosterFamily.getfFosterexperience() }</span>
					</div>
					<div class="information-row">
						<span class="information-row-title">可寄养的宠物类型：</span> <span>${fosterFamily.getfTyperange() }</span>
					</div>
					<div class="information-row">
						<span class="information-row-title">联系方式：</span> <span>${fosterFamily.getfContactway() }</span>
					</div>
					<div class="information-row">
						<span class="information-row-title">联系地址：</span> <span>${fosterFamily.getfAddress() }</span>
					</div>
				    <div class="information-row">
			...
			<div class="row">
				<div class="col-md-10 col-md-offset-1" style="margin-top: 20px;">
					...
					<textarea class="userInformation" name="psInformation"
						readonly="readonly">${systemInformation}</textarea>
				</div>

				<div class="col-md-10 col-md-offset-1" style="margin-top: 20px;">
					<form action="${pageContext.servletContext.contextPath }/feedbackinformation1.pet" method="post">
						<div>
							<span class="information-row-title"
								style="color: goldenrod; font-size: 20px;"> 填写反馈信息 </span>
						</div>
						<textarea class="userInformation" name="ffInformation"
							placeholder="可向系统提出反馈信息（如需要进行信息修改可填写申请~）"></textarea>
							
						<div class="prompt">${msg}</div>
						
						<input type="hidden" name="fId" value="${fosterFamily.getfId() }">
