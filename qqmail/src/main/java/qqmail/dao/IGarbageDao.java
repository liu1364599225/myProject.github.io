...
public interface IGarbageDao {
	public abstract List<MailPro> readAll_Garbage_Count(String garbage,int accept_id);//联表查询（状态表和邮件表）
	
	public abstract MailPro readAll_Garbage_ById(String garbage,int accept_id,int time);//查对应的单个邮件
	
	public abstract List<MailPro> readAll_Garbage(String garbage,int send_id,int begin,int end);//分页——联表查询（状态表和邮件表）
	
	public abstract List<MailPro> readAll_GarbagePro(String garbage,int send_id);//未读邮件数查询
